    ...
    try {
      const res = await authFetch('/api/admin/licenses', {
        ...
        body: JSON.stringify({
          ...
          expires_at_custom: durationType === 'custom' ? expiresAtCustom : undefined,
          notes,
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || err.message || 'Lisans oluşturulamadı.');
      }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500 transition-colors"
                  >
                    <option value="1_month">1 Ay (Geçici / Deneme)</option>
                    <option value="3_months">3 Ay</option>
                    <option value="6_months">6 Ay</option>
                    <option value="1_year">1 Yıl (Standart Paket)</option>
                    <option value="2_years">2 Yıl</option>
                    <option value="lifetime">Ömür Boyu (Sınırsız / Süresiz)</option>
                    <option value="custom">Özel Bitiş Tarihi Seç</option>
                  </select>

                  {durationType === 'custom' && (
                    <div className="mt-2">
                      <label className="block text-[11px] font-medium text-amber-300 mb-1">
                        Özel Bitiş Tarihi Seçin
                      </label>
                      <input
                        type="date"
                        required
                        ...
                        className="w-full bg-slate-950 border border-amber-500/40 rounded-xl px-3 py-2 text-sm text-amber-200 focus:outline-none focus:border-amber-400"
                      />
                    </div>
                  )}
                </div>
