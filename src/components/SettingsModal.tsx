...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleClearDemoData = async () => {
    setDataLoading(true);
    try {
      const res = await authFetch('/api/admin/clear-demo-data', {
        method: 'POST'
      });
      const data = await res.json();
      if (res.ok) {
        ...
        showToast('error', 'Hata', data.message || data.error || 'Veriler temizlenemedi.');
      }
    } catch (err) {
  ...
  const handleLoadDemoData = async () => {
    setDataLoading(true);
    try {
      const res = await authFetch('/api/admin/load-demo-data', {
        method: 'POST'
      });
      const data = await res.json();
      if (res.ok) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          ...
          {activeTab === 'info' && (
            <div className="space-y-4">
              ...
              <div className="bg-slate-950 p-4 border border-slate-800 rounded-2xl space-y-2 text-xs">
                <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">Cloudflare D1 & Sistem Bilgileri</h4>
                <div className="flex justify-between py-1 border-b border-slate-800">
                  <span className="text-slate-400">Yazılım Sürümü:</span>
                  <span className="text-indigo-400 font-semibold">ProPOS Central v2.4.0 (Cloudflare D1 Native)</span>
                </div>
                <div className="flex justify-between py-1 border-b border-slate-800">
                  <span className="text-slate-400">Cloudflare D1 Veritabanı ID:</span>
                  <span className="font-mono text-amber-300 font-bold select-all">93fe1eac-fc99-40f7-bcf7-3bfe3da4e548</span>
                </div>
                <div className="flex justify-between py-1 border-b border-slate-800">
                  <span className="text-slate-400">Veritabanı Adı (Binding DB):</span>
                  <span className="font-mono text-indigo-300 font-semibold">propos-license-db</span>
                </div>
                <div className="flex justify-between py-1 border-b border-slate-800">
                ...
                <div className="flex justify-between py-1 border-b border-slate-800">
                  <span className="text-slate-400">Yönetici Giriş Kullanıcı Adı:</span>
                  <span className="font-mono text-slate-200 font-bold">admin</span>
                </div>
      ...
      <ConfirmModal
        ...
        title="Örnek Verileri Sil"
        message="Tüm örnek veriler ve lisanslar silinecektir. Sistem temiz ve canlı kullanıma hazır duruma gelecektir. Onaylıyor musunuz?"
