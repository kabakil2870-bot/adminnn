...
export default function App() {
  ...
  useEffect(() => {
    const handleUnauthorized = () => {
      ...
      showToast('error', 'Oturum Süresi Doldu', 'Lütfen tekrar giriş yapın.');
    };
  ...
  const handleDeleteLicense = async (licenseId: string) => {
    try {
      const res = await authFetch(`/api/admin/licenses/${licenseId}`, {
        method: 'DELETE'
      });
      if (res.ok) {
  ...
  const handleDeleteDevice = async (deviceId: string) => {
    try {
      const res = await authFetch(`/api/admin/devices/${deviceId}`, {
        method: 'DELETE'
      });
      if (res.ok) {
  ...
  const handleToggleStatus = async (license: License) => {
    ...
    try {
      const res = await authFetch(`/api/admin/licenses/${license.id}`, {
        method: 'PATCH',
