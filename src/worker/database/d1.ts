...
export async function createLicense(db: D1Database, payload: CreateLicensePayload) {
  ...
  if (!licenseKey) {
    ...
    licenseKey = `PROPOS-${payload.max_devices}PC-${randomHex}-${randomHex2}-${year}`;
  }
...
export async function updateLicense(db: D1Database, id: string, payload: UpdateLicensePayload) {
  ...
  const clientPhone = payload.client_phone !== undefined ? payload.client_phone.trim() : existing.client_phone;
  const maxDevices = payload.max_devices !== undefined ? payload.max_devices : existing.max_devices;
  ...
  const expiresAt = payload.expires_at !== undefined ? payload.expires_at : existing.expires_at;
...
export async function deleteLicense(db: D1Database, id: string) {
  await db.prepare('DELETE FROM devices WHERE license_id = ?').bind(id).run();
