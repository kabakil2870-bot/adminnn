...
export async function initDbSchema(db: D1Database): Promise<void> {
  // Auto-detect if 'licenses' table exists with incompatible INTEGER id column and fix it
  try {
    const tableInfo = await db.prepare(`PRAGMA table_info(licenses)`).all<{ name: string; type: string; notnull: number }>();
    if (tableInfo.results && tableInfo.results.length > 0) {
      const idCol = tableInfo.results.find(c => c.name === 'id');
      if (idCol && idCol.type.toUpperCase().includes('INT')) {
        await db.prepare(`DROP TABLE IF EXISTS devices`).run();
        await db.prepare(`DROP TABLE IF EXISTS active_devices`).run();
        await db.prepare(`DROP TABLE IF EXISTS logs`).run();
        await db.prepare(`DROP TABLE IF EXISTS activation_logs`).run();
        await db.prepare(`DROP TABLE IF EXISTS licenses`).run();
      }
    }
  } catch (e) {
    console.error('Error checking licenses table compatibility:', e);
  }
