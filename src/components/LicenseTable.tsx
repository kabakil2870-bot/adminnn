...
export const LicenseTable: React.FC<LicenseTableProps> = ({
  ...
  const [confirmDeleteLicense, setConfirmDeleteLicense] = useState<License | null>(null);
  const [confirmDeleteDevice, setConfirmDeleteDevice] = useState<Device | null>(null);
  const [confirmToggleDevice, setConfirmToggleDevice] = useState<Device | null>(null);
...
ProPOS Otomasyon`;
  ...
  const executeToggleDeviceBlockStatus = async (dev: Device) => {
    const targetStatus = dev.status === 'blocked' ? 'active' : 'blocked';

    try {
      const res = await authFetch(`/api/admin/devices/${dev.id}/status`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden">
                                      ...
                                      <tr key={dev.id} className="hover:bg-slate-800/30 transition-colors">
                                        ...
                                        <td className="py-2.5 px-3 text-right font-sans">
                                          <div className="flex items-center justify-end gap-2">
                                            {/* Block/Unblock */}
                                            <button
                                              onClick={() => setConfirmToggleDevice(dev)}
      ...
      <ConfirmModal
        ...
        title="Lisansı Sil"
        message={`'${confirmDeleteLicense?.client_name}' firmasına ait lisansı ve buna kayıtlı tüm cihaz tanımlarını silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`}
        confirmText="Evet, Lisansı Sil"
        cancelText="Vazgeç"
        variant="danger"
        onConfirm={() => {
      ...
      <ConfirmModal
        isOpen={!!confirmDeleteDevice}
        title="Cihaz Kaydını Sil"
        message={`"${confirmDeleteDevice?.device_name || confirmDeleteDevice?.hardware_id}" cihazının kaydını silmek istediğinize emin misiniz? Bu işlem lisans için yeni bir PC slotu açacaktır.`}
        confirmText="Evet, Cihazı Sil"
        cancelText="Vazgeç"
        variant="danger"
        onConfirm={() => {
          if (confirmDeleteDevice) {
            onDeleteDevice(confirmDeleteDevice.id);
      ...
      {/* Toggle Device Confirm Modal */}
      <ConfirmModal
        isOpen={!!confirmToggleDevice}
        title={confirmToggleDevice?.status === 'blocked' ? 'Cihaz Engeli Kaldır' : 'Cihazı Engelle'}
        message={`"${confirmToggleDevice?.device_name || confirmToggleDevice?.hardware_id}" cihazının durumunu ${
          confirmToggleDevice?.status === 'blocked' ? "'Aktif'" : "'Engelli'"
        } yapmak istediğinize emin misiniz?`}
        confirmText={confirmToggleDevice?.status === 'blocked' ? 'Engeli Kaldır' : 'Engelle'}
        cancelText="Vazgeç"
        variant={confirmToggleDevice?.status === 'blocked' ? 'info' : 'warning'}
        onConfirm={() => {
          if (confirmToggleDevice) {
            executeToggleDeviceBlockStatus(confirmToggleDevice);
            setConfirmToggleDevice(null);
          }
        }}
        onCancel={() => setConfirmToggleDevice(null)}
      />
    </div>
  );
};
