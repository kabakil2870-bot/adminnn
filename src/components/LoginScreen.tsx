...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess, showToast }) => {
  ...
  const [password, setPassword] = useState('54635463m.M');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        onLoginSuccess(data.token, data.admin?.username || username);
        showToast('success', 'Giriş Başarılı', 'Cloudflare Workers & D1 Lisans Yönetim Paneline hoş geldiniz.');
      } else {
        showToast('error', 'Giriş Başarısız', data.error || data.message || 'Kullanıcı adı veya şifre yanlış.');
      }
    } catch (err) {
  ...
  const quickDemoLogin = async () => {
    setUsername('admin');
    setPassword('54635463m.M');
    setLoading(true);
    try {
      const res = await fetch('/api/admin/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: 'admin', password: '54635463m.M' })
      });
      const data = await res.json();
      if (res.ok && data.token) {
        ...
        showToast('error', 'Giriş Hatası', data.error || 'Admin girişi yapılamadı.');
      }
    } catch (err) {
      showToast('error', 'Hata', 'Giriş isteği başarısız oldu.');
    } finally {
      setLoading(false);
    }
  };
