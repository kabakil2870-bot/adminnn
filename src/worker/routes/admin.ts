...
export async function handleAdminRoutes(request: Request, env: Env, url: URL): Promise<Response | null> {
  ...
  if (path.startsWith('/api/admin/')) {
    ...
    if (deleteLicMatch && request.method === 'DELETE') {
      const id = decodeURIComponent(deleteLicMatch[1]);
      try {
        let existing = await getLicenseById(env.DB, id);
        if (!existing) {
          existing = await getLicenseByKey(env.DB, id);
        }
        if (!existing) return errorResponse('Lisans bulunamadı.', 404);

        await deleteLicense(env.DB, existing.id);
        ...
        return errorResponse('Silme hatası: ' + err.message, 500);
      }
    }
    ...
    if (deleteDevMatch && request.method === 'DELETE') {
      const deviceId = decodeURIComponent(deleteDevMatch[1]);
      try {
        ...
        return errorResponse('Cihaz silme hatası: ' + err.message, 500);
      }
    }
